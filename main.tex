...
\begin{frame}{Nested Classes in Newspeak}
\includegraphics[width=\textwidth]{newspeak_basic.pdf}
\end{frame}

\begin{explainframe}{Nested Classes in Newspeak}
\begin{itemize}
...
\begin{enumerate}
  ...
  \item Generate class objects for nested classes (s.t. optimizations)
\end{enumerate}

\begin{lstlisting}
Object subclass: #CombinatorialParsing
    instanceVariableNames: 'CombinatorialParser SequentialParser ... OrderedCollection LinkedList'.

CombinatorialParsing class>>usingPlatform: platform
  | inst |
  inst OrderedCollection: platform collections OrderedCollection.
  inst LinkedList: platform collections LinkedList.
  inst CombinatorialParser: Object subclass: ...
  inst CombinatorialParser compile: 'and: parser ^ parent OrderedCollection with: self with: parser'.
  ...
  ^ inst 
\end{lstlisting}
(getters and setters exist for all instance variables)
\end{frame}

\begin{frame}{Class Hierarchy Inheritance in Newspeak}
\centering
